...
function UserDescriptiveTable() {
  ...
  const fetchSectionData = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8000/api/v1/user/getuserdescriptiveperfomance/${userId}/${sessionId}`
      );
      const questionDetails =
        response.data.data.sessionDetails.descriptiveAnswers;
  ...
  const downloadCSV = () => {
    ...
    questions.forEach((item, index) => {
      const status =
        item.markObtained !== undefined && item.markObtained > 0
          ? "Answered"
          : "Skipped";
            ...
            <tbody className={styles.tbody}>
              {questions.map((item, index) => (
                <tr key={index}>
                  ...
                  <td>
                    {item.answerText !== "skipped" ? "Answered" : "Skipped"}
                  </td>
                </tr>
              ))}
            </tbody>
